 ...
 * 1. Full provenance triple (sourceSystem + sourceId + sourceClaimId) -> upsert (increment mentionCount)
 * 2. Content-based fallback (contactId + interest text + sourceSystem + sourceId) -> upsert (increment mentionCount)
  ...
  try {
    ...
    if (body.sourceSystem && body.sourceId) {
      const contentMatch = await prisma.personalInterest.findFirst({
        where: { contactId: body.contactId, interest: body.interest.trim(), sourceSystem: body.sourceSystem, sourceId: body.sourceId },
      })
      if (contentMatch) {
        const updated = await prisma.personalInterest.update({
          where: { id: contentMatch.id },
          data: { mentionCount: contentMatch.mentionCount + 1, lastMentioned: new Date().toISOString() },
        })
        return NextResponse.json({ ...updated, action: "updated" }, { status: 200 })
      }
    }
    ...
    const interest = await prisma.personalInterest.create({
      data: {
        contactId: body.contactId, interest: body.interest.trim(),
        confidence: body.confidence || "medium", source: body.source || "manual",
        sourceSystem: body.sourceSystem || null, sourceId: body.sourceId || null, sourceClaimId: body.sourceClaimId || null,
      },
    })
    return NextResponse.json({ ...interest, action: "created" }, { status: 201 })
  } catch (error) {
